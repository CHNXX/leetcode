...
class twoSum {
    public static void main(String[] args) {
        twoSum tow = new twoSum();
        System.out.println(tow.twoSum(new int[]{0,4,3,0},3).toString());;
    }
    public int[] twoSum(int[] nums, int target) {
        HashMap<Integer, Integer> numsMap = new HashMap<>(nums.length);
        for (int i = 0; i < nums.length; i++) {
                if (numsMap.containsKey(target - nums[i])) {
                    int[] rst = new int[]{i,numsMap.get(target-nums[i])};
                    rst = Arrays.stream(rst).sorted().toArray();
                    return rst;
                } else {
